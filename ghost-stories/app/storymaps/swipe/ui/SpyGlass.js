define(["dojo/_base/declare", 
		...
		) 
	{
		...
		var LENS = declare([WidgetBase, TemplatedMixin, WidgetsInTemplateMixin], {
			...
			startup: function(webmapId, lensNode, mapContainerNode, map, colors, titles, mode, layers, lensCenterOffset, popup)
			{			
				_this = this;
				...
				$('#infoWindowTitle').html("No Feature");
				$('#infoWindowContent').html('<div class="noFeature">No Feature Explanation</div>');
				...
				if(_layers && _layers.length && _this.map.getLayer(_layers[0]))
					_this._isGraphics = _this.map.getLayer(_layers[0]).type == "Feature Layer";
				...
				var handleLensClick = function(evt){
					setTimeout(function(){
						...
						evt.type = "click";
						evt.fromLens = true;
						evt.mapPoint = _this.map.toMap(new Point(x, y, _this.map.spatialReference));
						evt.graphic = null;
						
						_this.map.onClick(evt, "other");
						_this._moving = false;
				...
				on(_this.map, "click", function(evt){
			...
			syncLensExtent: function() 
			{
				...
				var ll = _this.map.toMap(new Point(bb.l, bb.t + 400, _this.map.spatialReference));
				// upper-right x and y
				var ur = _this.map.toMap(new Point(bb.l + 400, bb.t, _this.map.spatialReference));
				_spyExtent = new Extent(ll.x, ll.y, ur.x, ur.y, _this.map.spatialReference);
	...
	function setInfoWindow(feature, popupIndex, map2Click)
	{
		if( feature )
			...
			registry.byId("infoView-mobile" + popupIndex).set("content", '<div class="noDataMap"><div class="title">No Data</div></div>');
